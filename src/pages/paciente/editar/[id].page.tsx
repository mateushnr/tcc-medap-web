...
} from '@/pages/profissional/editar/styles'
import {
  ContainerCancelAction,
  EditingLabel,
} from '@/pages/estabelecimento/editar/styles'
import { useRouter } from 'next/router'

export interface CustomerEditedDataToSendProps {
  name: string
  documentType?: string
  cpf?: string
  ...
  patientsResponsibleIdListToAdd?: string[]
  patientsResponsibleIdListToRemove?: string[]
  petsList?: Pet[]
}
...
interface EditCustomerProps {
  customerToEditData: ResponseCustomerWithAddress | null
...
export default function EditCustomer({
  ...
  const [statusSelectData, setStatusSelectData] =
    useState<selectData[]>(customerStatus)
  ...
  const [
    currentPatientsResponsibleIdList,
    setCurrentPatientsResponsibleIdList,
  ...
  const [isPatientInputKey, setIsPatientInputKey] = useState<number>(0)

  const [isPatientChecked, setIsPatientChecked] = useState<boolean>(
    customerToEditData?.isPatient || false,
  )
  ...
  const handleGetPatientsFromEstablishmentSelected = useCallback(
    async (idEstablishmentSelected: string) => {
      ...
      const filteredPatientsList = patientsList?.filter((patient) => {
        if (
          customerToEditData?.patientResponsibleList &&
          customerToEditData?.patientResponsibleList.length > 0
        ) {
          return !customerToEditData?.patientResponsibleList.includes(
            patient.value,
          )
        } else {
          return true
      ...
      setSelectPatientsListData(filteredPatientsList || null)
      setPatientsListData(patientsList)
      setSelectPatientListInputKey((prev) => prev + 1)

      setCurrentPatientsResponsibleIdList(
        customerToEditData?.patientResponsibleList || null,
      )
      ...
      if (
        ...
        const patientResponsibleSelectData =
          customerToEditData.patientResponsibleList.map((patientId) => {
            const patient = patientsList?.find((p) => p.value === patientId)
        ...
        setPatientsResponsibleSelectedList(patientResponsibleSelectData)
      }
    },
    [customerToEditData],
  )

  const fillAllFieldsWithCustomerData = useCallback(() => {
    if (customerToEditData) {
      setCustomerValue('name', customerToEditData.name)
      setCustomerValue('email', customerToEditData.email || '')
      setCustomerValue('cpf', customerToEditData.cpf || '')
      setCustomerValue('otherDocument', customerToEditData.otherDocument)
      setCustomerValue('cns', customerToEditData.cns)
      setCustomerValue('documentType', customerToEditData.cpf ? 'CPF' : 'OTHER')
      setDocumentIdentifierTypeSelected(
        customerToEditData?.cpf ? 'CPF' : 'OTHER',
      )
      setCustomerValue('birthDate', customerToEditData.birthDate)
      setCustomerValue('mainPhone', customerToEditData.mainPhone)
      setCustomerValue('secondaryPhone', customerToEditData.secondaryPhone)
      setCustomerValue('isPatient', customerToEditData.isPatient)
      setIsPatientChecked(customerToEditData.isPatient)

      setIsPatientInputKey((prev) => prev + 1)
      setCustomerValue('isResponsible', customerToEditData.isResponsible)
      setCustomerValue('isTutor', customerToEditData.isTutor)
      setCustomerValue('status', customerToEditData.status)
      setCustomerValue(
        'establishmentBounded',
        customerToEditData.customerEstablishment,
      )

      setCustomerValue('postCode', customerToEditData.postCode)
      setCustomerValue('street', customerToEditData.street || '')
      setCustomerValue('neighborhood', customerToEditData.neighborhood || '')
      setCustomerValue('city', customerToEditData.city || '')
      setCustomerValue('state', customerToEditData.state || '')
      setCustomerValue('compliment', customerToEditData.compliment || '')
      setCustomerValue('number', customerToEditData.number || undefined)
      setCustomerValue('latitude', customerToEditData.latitude || undefined)
      setCustomerValue('longitude', customerToEditData.longitude || undefined)

      setIsEstablishmentSelected(true)

      handleGetPatientsFromEstablishmentSelected(
        customerToEditData?.customerEstablishment,
      )

      setStatusSelectData((prev) => {
        return prev.map((option) => {
          return option.value === customerToEditData.status
      ...
      setStateSelectData((prev) => {
        return prev.map((option) => {
          return option.value === customerToEditData.state
            ? { text: option.text, value: option.value, selected: true }
            : option
        })
      })
  ...
  const handleEditCustomer = async (data: EditCustomerData) => {
    const { 'medap.token': token } = parseCookies()
    setCreatedProfessionalFeedbackMessage(null)

    const patientsResponsibleIdListToAdd = patientsResponsibleSelectedList
      .filter(
        (patient) => !currentPatientsResponsibleIdList?.includes(patient.value),
      )
      .map((patient) => patient.value)

    const patientsResponsibleIdListToRemove =
      currentPatientsResponsibleIdList?.filter(
        (patientId) =>
          !patientsResponsibleSelectedList.some(
    ...
    const customerEditedDataToSend: CustomerEditedDataToSendProps = {
      ...data,
      patientsResponsibleIdListToAdd,
      patientsResponsibleIdListToRemove,
      petsList,
      isResponsible: patientsResponsibleSelectedList.length > 0,
      isTutor: customerToEditData?.isTutor || petsList.length > 0,
    }

    delete customerEditedDataToSend.documentType
    delete customerEditedDataToSend.patientSelected

    try {
      const response = await fetch(
        `http://localhost:3000/api/customers/?id=${router.query.id}`,
        {
          method: 'PUT',
          headers: {
          ...
          body: JSON.stringify(customerEditedDataToSend),
        },
      )
  ...
  return (
    <DefaultLayout activePage="PATIENT">
      <HeaderUser title="Editar cliente" />
      <CreateProfessionalContainer>
        <Box width={'full'}>
          <ProfessionalBeingEditedContainer>
            <ProfessionalInfoContainer>
              ...
              <ProfessionalNameInfo>
                <Text color={'gray_700'} size={'2xlarge'}>
                  {customerToEditData?.name}
                </Text>
                <Text color={'gray_600'} size={'xlarge'}>
                  {customerToEditData?.cpf
                    ? customerToEditData?.cpf
                    : customerToEditData?.otherDocument}
                </Text>
              </ProfessionalNameInfo>
            </ProfessionalInfoContainer>
            <ContainerCancelAction>
          ...
          <CreateProfessionalForm
            onSubmit={handleCustomerSubmit(handleEditCustomer)}
          >
            <FormContainer>
              <ProfessionalData>
                <ProfessionalDataHeader>
                ...
                <ProfessionalDataFields>
                  ...
                  <CheckboxContainer>
                    <Checkbox
                      key={isPatientInputKey}
                      id="isPatient"
                      type="checkbox"
                      value="isPatient"
                      onChange={() => {
                        setIsPatientChecked((prev) => !prev)
                        setCustomerValue('isPatient', !isPatientChecked)
                      }}
                      checked={isPatientChecked}
                    />
...
export const getServerSideProps: GetServerSideProps = async (context) => {
  ...
  let customerToEditData: EditCustomerData | null = null
  ...
  if (user != null) {
    ...
    if (routeParams) {
      try {
        const response = await fetch(
          `http://localhost:3000/api/customers/?id=${routeParams.id}`,
          {
            method: 'GET',
            headers: {
              'Content-Type': 'application/json',
        ...
        if (response.ok) {
          customerToEditData = await response.json()
        } else {
